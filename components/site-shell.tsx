...
export function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    {children}
    <footer className="border-t border-border px-5 py-8 md:px-10"><div className="mx-auto flex max-w-[1440px] flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><p className="font-serif text-2xl tracking-[-.04em]">WUCH FILMS</p><p className="mt-2 text-xs text-muted-foreground">Independent moving image.</p></div><div className="flex gap-5 text-[10px] uppercase tracking-[.18em] text-muted-foreground"><a href="mailto:hello@wuchfilms.com">Email</a><a href="#">Instagram</a><span>© {new Date().getFullYear()}</span></div></div></footer>
  </div>
}

export function FilmImage({ src, alt, className = '' }: { src: string; alt: string; className?: string }) { return <div className={`relative overflow-hidden bg-muted ${className}`}><Image src={src} alt={alt} fill sizes="(max-width: 768px) 100vw, 70vw" className="object-cover transition-transform duration-700 hover:scale-[1.03]" /></div> }
